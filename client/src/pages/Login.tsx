...
export default function Login() {
  ...
  // Check if already logged in as freelancer
  const freelancerMe = trpc.freelancer.me.useQuery(undefined, { retry: false });

  useEffect(() => {
    if (freelancerMe.data) {
      navigate("/dashboard");
    }
  }, [freelancerMe.data]);

  const loginMutation = trpc.freelancer.login.useMutation({
    onSuccess: (data) => {
      toast.success(`Welcome back, ${data.freelancer.name}!`);
      if (data.freelancer.status === "pending") {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <img src={LOGO_URL} alt="Lingora" className="h-12 w-auto mx-auto mb-3" />
          <h1 className="text-2xl font-bold text-foreground">Vendor Sign In</h1>
          <p className="text-muted-foreground text-sm mt-1">Sign in to your Lingora vendor account</p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => toast.info("Password reset coming soon. Contact support@lingoraloc.com")}
        ...
        <p className="text-center text-xs text-muted-foreground mt-3">
          ...
          <a href="mailto:a.ashraf@lingoraloc.com" className="text-primary hover:underline">
            a.ashraf@lingoraloc.com
          </a>
        </p>

        <p className="text-center text-xs text-muted-foreground mt-2">
          Admin?{" "}
          <a href="/admin/login" className="text-muted-foreground hover:text-foreground underline">
            Sign in here
          </a>
        </p>
      </div>
